...
dbutils.fs.mkdirs("dbfs:/FileStore/streamDir")
...
for (x<-Seq.range(1,41))
  dbutils.fs.cp(s"/databricks-datasets/structured-streaming/events/file-$x.json",s"dbfs:/FileStore/streamDir/file-$x.json")

// COMMAND ----------

// MAGIC %fs ls dbfs:/FileStore/streamDir
...
val ilAkcji = staticInputDF.groupBy($"action",window($"time","1 hour")).count()

ilAkcji.createOrReplaceTempView("ilAkcji")
...
val query = streamingCountsDF.writeStream.format("memory")
    .outputMode("complete")
    .queryName("countAction")
    .start()
...
// MAGIC %sql select action, date_format(window.end, "MMM-dd HH:mm") as time, count from countAction order by time, action
...
query.stop() // zatrzymanie streamu
